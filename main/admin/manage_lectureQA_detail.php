	...
	<div class="container">
		...
		<div class="contwrap has_fixed_title">
			...
			<table id="">
				...
				<tbody>
					...
					?>
							...
							<input class="qna_idx" type="hidden" data-idx="<?=$qa['idx']?>"/>
					<?php
								}
					?>
						</td>
			...
			<div class="btn_wrap">
                ...
                <button type="button" class="border_btn confirm">저장</button>
			</div>
		</div>
	</div>
</section>
<script>
	$('.confirm').click(function(){
		if (confirm("저장하시겠습니까?")) {
            var temp_this
            var li_qna_arr = new Array();
                        
            $('.qna_idx').each(function(i){
                temp_this =$(this).parent();
                li_qna_arr.push(temp_this.find('.qna_idx').data('idx') + "|" + temp_this.find("input[type=radio]:checked").val());
            });
			$.ajax({
				url : "../ajax/admin/ajax_lecture.php",
				type : "POST",
				data : {
					flag : "confirm_qna_arr",
					li_qna_arr : li_qna_arr.join('^&')
				},
				dataType : "JSON",
