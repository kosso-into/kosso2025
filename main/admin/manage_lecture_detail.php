...
//	include_once('../live/include/set_event_period.php');
	include_once($_SERVER['DOCUMENT_ROOT']."/main/include/set_event_period.php");
				...
				<table>
					...
					<tbody>
						...
						<tr>
							...
							<td class="clearfix">
								<!-- select -->
								<select class="w50 tag_input floatL s_join" name="chairperson">
									<option value="">Chairperson 선택</option>
									<?php
										...
										if ($is_modify) {
											if (count($info['speakers']) > 0) {
												foreach($info['speakers'] as $sp){
                                                    ...
                                                    if($sp['s_type']==1){
									?>
									<li data-type="" data-idx="<?=$sp['idx']?>" data-val="<?=$sp['val']?>" data-cat="1"><?=$sp['name_en']?><i class="delete_i"></i></li>
									<?php
                                                    }
												}
											}
						...
						<tr>
							...
							<td class="clearfix">
								<!-- select -->
								<select class="w50 tag_input floatL s_join" name="speaker">
										...
										if ($is_modify) {
											if (count($info['speakers']) > 0) {
												foreach($info['speakers'] as $sp){
                                                    ...
                                                    if($sp['s_type']==2){
									?>
									<li data-type="" data-idx="<?=$sp['idx']?>" data-val="<?=$sp['val']?>" data-cat="2"><?=$sp['name_en']?><i class="delete_i"></i></li>
									<?php
                                                    }
												}
											}
							...
							<td class="clearfix">
								<!-- select -->
								<select class="w50 tag_input floatL s_join" name="panel">
									<option value="">Panel 선택</option>
									<?php
										foreach($speaker_list as $sp){
											foreach($sp as $key=>$value){
												$sp[$key] = htmlspecialchars_decode($value);
										...
										if ($is_modify) {
											if (count($info['speakers']) > 0) {
												foreach($info['speakers'] as $sp){
                                                    ...
                                                    if($sp['s_type']==3){
									?>
									<li data-type="" data-idx="<?=$sp['idx']?>" data-val="<?=$sp['val']?>" data-cat="3"><?=$sp['name_en']?><i class="delete_i"></i></li>
									<?php
                                                    }
												}
											}
...
<script>
	...
	// speaker 추가
	$('.s_join').change(function(){
		var dup_flag = false;
		var exist_cnt = 0;
        var temp_this = $(this).attr('name');
		var selected_opt = $('[name='+temp_this+'] option:selected');
		$('[name='+temp_this+']+.tag_list li').each(function(){
			if ($(this).data('val') == selected_opt.val()) {
				dup_flag = !dup_flag;
				return;
			}
        ...
        if(temp_this == 'chairperson'){
            var data_cat = 1;
        }else if(temp_this == 'speaker'){
            var data_cat = 2;
        }else if(temp_this == 'panel'){
            var data_cat = 3;
        }
		$('[name='+temp_this+']+.tag_list').append('<li data-type="insert" data-idx="" data-val="' + selected_opt.val() + '" data-cat="' + data_cat + '">' + selected_opt.text() + '<i class="delete_i"></i></li>');
	});
	...
	$('.save_btn').click(function(){
		...
		$('.s_join+.tag_list li').each(function(){
			temp_this = $(this);
			if (temp_this.data("type") != "delete") {
				li_speaker_exist = true;
			}
			li_speaker_arr.push(temp_this.data("type") + "|" + temp_this.data("idx") + "|" + temp_this.data("val") + "|" + temp_this.data("cat"));
		});
