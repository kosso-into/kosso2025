<?php
	...
	// 수정(+생성)
        ...
        error_log(print_r($li_speaker_arr,true));
		if (count($li_speaker_arr) > 0) {
			foreach($li_speaker_arr as $li){
				list($type, $idx, $val, $cat) = explode('|', $li);

				if ($type !== "") {
					if ($type == "insert") { // insert
						$sql =	"INSERT INTO
									lecture_speaker_join
									(lecture_idx, speaker_idx, speaker_type)
								VALUES
									('".$lecture_idx."', '".$val."', '".$cat."')
								";
					} else { // delete
	...
	// 질의 - 승인
	else if ($flag === "confirm_qna_arr") {
        // qna
        $li_qna_arr = explode('^&', htmlspecialchars_decode($_POST['li_qna_arr']));
        if (count($li_qna_arr) > 0) {
            foreach($li_qna_arr as $li){
                list($idx, $status) = explode('|', $li);
                if ($idx !== "") {
                    $sql =	"UPDATE lecture_qna
                            SET
                                confirm_yn = '".$status."'
                ...
                if (!sql_query($sql)) {
                    return_value(500, "QNA 정보 저장 중 오류가 발생했습니다.\n관리자에게 문의하세요.");
                }
            }
        }
		return_value(200, "완료되었습니다.");
