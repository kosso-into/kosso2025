	...
	$sql_place =	"SELECT
					...
					FROM (
						...
						LEFT JOIN (
							SELECT
								lsj.lecture_idx,
								GROUP_CONCAT(CONCAT(ls.name_en, ' (', ls.affiliation_en, ')') ORDER BY lsj.idx separator '|') AS name_en_text,
								GROUP_CONCAT(lsj.speaker_type ORDER BY lsj.idx separator '|') AS speaker_type,
								GROUP_CONCAT(
									CONCAT(
										IFNULL(DATE_FORMAT(ls.lecture_start_time, '%H:%i'), 'null'),                                         
										' - ', 
										IFNULL(DATE_FORMAT(ls.lecture_end_time, '%H:%i'), 'null')
									) ORDER BY lsj.idx separator '|') AS lecture_period,
								GROUP_CONCAT(IFNULL(ls.lecture_title, 'null') ORDER BY lsj.idx separator '|') AS lecture_title
...
?>
		...
		<?php include_once ($_SERVER['DOCUMENT_ROOT']."/main/include/popup_mypage.php");?>
	...
	<script>
		...
		$(".card_list li").mouseover(function(){
			...
			if (place_info.lecture_idx > 0) {
                ...
                var speaker_type = place_info.speaker_type.split('|');
                var speaker_name = place_info.lecture_chairperson_name_en.split('|');
                var speaker_period = place_info.lecture_period.split('|');
                var speaker_title = place_info.lecture_title.split('|');
		        var chairperson_arr = new Array();
		        var speaker_arr = new Array();
		        var panel_arr = new Array();
                var period_arr = new Array();
                var title_arr = new Array();
                for (var i=0; i<speaker_type.length; i++) {
                    if(speaker_type[i]==1){
                        chairperson_arr.push(speaker_name[i]);
                        var inner = '';
                        for (var j=0; j<chairperson_arr.length; j++) {
                            if (j > 0) {
                                inner += ', ';
                            }

                            inner += chairperson_arr[j];
                        }
					   $('.lecture_info_white [name=chairperson]').html(inner);
                    }else if(speaker_type[i]==2){
                        speaker_arr.push(speaker_name[i]); 
                        period_arr.push(speaker_period[i]);
                        title_arr.push(speaker_title[i]);
                        
                        var inner = '';
                        for (var j=0; j<speaker_arr.length; j++) {
                            inner +=	'<li>';
                            inner +=		'<span>' + period_arr[j] + '</span>';
                            inner +=		'<span>' + title_arr[j] + '</span>';
                            inner +=		'<span>' + speaker_arr[j];
...
//                            }
                    ...
                    }else if(speaker_type[i]==3){
                        panel_arr.push(speaker_name[i]);  
                        var inner = '';
                        for (var j=0; j<panel_arr.length; j++) {
                            if (j > 0) {
                                inner += ', ';
                            }
