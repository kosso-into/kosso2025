<?php
	...
	$sql_schedule =	"SELECT
						...
						IFNULL(lc_sp.name_en, '') AS speakerName,
						IFNULL(lc_sp.affiliation_en, '') AS speakerAffiliation,
						IFNULL(lc_sp.speaker_type, '') AS speaker_type,
                        lc_sp.lecture_period as lecture_period,
                        lc_sp.lecture_title as lecture_title,
					...
					LEFT JOIN (
						...
						INNER JOIN (
							SELECT
                                ...
                                lecture_start_time, lecture_end_time, lecture_title
...
?>
		...
		<script>
			document.addEventListener('DOMContentLoaded', function() {
		        ...
		        var chairperson_Narr = new Array();
		        var chairperson_Aarr = new Array();
		        var panel_Narr = new Array();
		        var panel_Aarr = new Array();
                console.log(schedules);
				schedules.forEach(function(el){
                    var speaker_type = el.speaker_type.split('|');
                    var speaker_name = el.speakerName.split('|');
                    var speaker_affiliation = el.speakerAffiliation.split('|');
                    for (var i=0; i<speaker_type.length; i++) {
                        if(speaker_type[i]==1){
                            chairperson_Narr.push(speaker_name[i]);
                            chairperson_Aarr.push(speaker_affiliation[i]);
                            // chair person
                            var chairPersonObj = {
                                names			: chairperson_Narr,
                                affiliations	: chairperson_Aarr,
                                data_length		: 0,
                                text			: ""
                            };
                    ...
                    console.log(chairperson_Narr);
                    console.log(chairperson_Aarr);
//					// chair person
//					var chairPersonObj = {
